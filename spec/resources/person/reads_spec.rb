...
RSpec.describe PersonResource, type: :resource do
  ...
  let!(:user_role) {
    Fabricate(Group::GremiumProjektgruppe::Leitung.name, person: Fabricate(:person),
      group: groups(:berner_gremium))
  }
  ...
  describe "serialization" do
    let!(:person) { Fabricate(:person, birthday: Date.current, gender: "m") }
    let!(:role) {
      Fabricate(Group::GremiumProjektgruppe::Mitglied.name, person: person,
        group: groups(:berner_gremium))
    }
  ...
  describe "sideloading" do
    describe "updated_by" do
      let!(:updater_person) {
        Fabricate(Group::GremiumProjektgruppe::Mitglied.name, group: groups(:berner_gremium)).person
      }
      let!(:updated_person) { Fabricate(:person, updater_id: updater_person.id) }
      let!(:role) {
        Fabricate(Group::GremiumProjektgruppe::Mitglied.name, person: updated_person,
          group: groups(:berner_gremium))
      }
      let(:person) {
        updater_person
      } # required by core ResourceSpecHelper which is included for resource specs
