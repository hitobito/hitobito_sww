...
describe Oauth::ProfilesController do
  let(:benutzerkonten) {
    Group::Benutzerkonten.create!(name: "CMS Benutzer", parent: groups(:schweizer_wanderwege))
  }
  let(:cms_benutzer) do
  ...
  let(:token) {
    app.access_tokens.create!(resource_owner_id: cms_benutzer.id, scopes: "name email with_roles",
      expires_in: 2.hours)
  }
